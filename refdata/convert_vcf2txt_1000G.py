...
# Description: Convert .vcf to .txt file of a reference data to be used in iAdmix. 
...
def get_superpopulation_allele_frequency( inputfile ):
	with open(inputfile) as fh:
		...
		for line in fh:
			...
			# Skip if non-canonical chromosome e.g. alt random Un
			if any(re.findall(r'alt|random|Un', line[0])):
				continue

			# Skip if INDEL or multi-allelic
			if any(re.findall(r'INDEL|MULTI_ALLELIC', line[7])):
				continue

			# Skip if the variant type is not SNP
			if ( line[7].find("VT=SNP") < 0 ): 
				continue
...
def usage():
	print 'Usage: python convert_vcf2txt_1000G.py -i <inputfile>'
